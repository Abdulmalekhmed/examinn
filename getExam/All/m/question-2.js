...
const createContactList = (
  ...
  emailGetter = (item) => item.email
) => {
  return list.map(form => {
    return {
      name: nameGetter(form),
      email: emailGetter(form)
    }
  })
...
const testConvertedSpanishData = createContactList(
  spanishTestData,
  (item) => item.nombre,
  (item) => item.mail
);
...
console.assert(
  testConvertedSpanishData[1].email === "wouter@hackyourfuture.net"
);

const testConvertedDutchData = createContactList(
  dutchTestData,
  (item) => item.naam,
  (item) => item.emailadres
);
...
console.assert(
  testConvertedDutchData[1].email === "federico@hackyourfuture.net"
);