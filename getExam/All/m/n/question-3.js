...
const hasPassed = (
  ...
  exceptionInfo = defaultExceptionInfo
) => {
  const { dyslexia, adhd } = exceptionInfo;

  if (!passedPractical) return false;

  if (!dyslexia && !adhd) {
    return scoreTheory >= 90;
  } else if (dyslexia && adhd) {
    return scoreTheory >= 70;
  } else {
    return scoreTheory >= 80;
  }
};
...
console.assert(
  hasPassed(80, true, { dyslexia: true }) === true,
  "80, true, dyslexia"
); // Dyslexia exception
console.assert(
  hasPassed(79, true, { dyslexia: true }) === false,
  "79, true, dyslexia"
); // Dyslexia exception only allows 80+
...
console.assert(
  hasPassed(70, true, { dyslexia: true, adhd: true }) === true,
  "70, true, dyslexia/adhd"
); // ADHD and dyslexia exception
console.assert(
  hasPassed(69, true, { dyslexia: true, adhd: true }) === false,
  "69 ,true, dyslexia/adhd"
); // ADHD and dyslexia exception only allows 70+
