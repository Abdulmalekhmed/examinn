 ...
 *
 * The function will return a NEW array of todos where the `completedDate` and `isCompleted` fields are correctly altered. The `completedDate` needs to be filled with the current time, `isCompleted` needs to be set to true
 */
const completeMultipleTodos = (allTodos, todoIdsToComplete) => {
  return allTodos.map((todo) => {
    const newTodo = { ...todo };
    if (todoIdsToComplete.includes(todo.id)) {
